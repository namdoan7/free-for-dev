<?php
    ...
    if (DEVELOPMENT) {
        ...
        foreach ($handler AS $entry) {
            if ($entry != '.' && $entry != '..' && $entry != basename(PATH_CONFIG) && $entry != basename(DEVELOPMENT_FILE) && $entry != basename(DEVELOPMENT_INC) && $entry != basename(VERSION_INC) && is_file('./' . $entry)) {
                $files[] = $entry;
                $times[] = filemtime('./' . $entry);
            }
        }

        unset($handler);

        if (is_file(DEVELOPMENT_FILE)) {
        ...
        if ($isCreator)
            file_put_contents(DEVELOPMENT_FILE, jsonEncode(array('files' => $files, 'times' => $times, 'count' => $count, 'version' => $version)));

        if ($isCreator || $isModifier)
            file_put_contents(VERSION_INC, '<?php if (!defined(\'ACCESS\')) { die(\'Not acces\'); } else { $count = ' . $count . '; $version = \'' . $version . '\'; } ?>');
    } else if (is_file(VERSION_INC)) {
        require_once VERSION_INC;
    }
    ...
    if (!DEVELOPMENT && is_file(DEVELOPMENT_FILE))
